...
<div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden max-w-2xl">
    ...
    <form action="{{ route('requirements.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-5">
        @csrf
        ...
        <div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                <label class="flex items-center gap-2 px-4 py-3 border-2 rounded-xl cursor-pointer transition-all {{ old('type', 'document') == 'document' ? 'border-amber-500 bg-amber-50' : 'border-slate-200' }}">
                    <input type="radio" name="type" value="document" {{ old('type', 'document') == 'document' ? 'checked' : '' }} class="w-4 h-4">
                    <span><i class="ph ph-file-text text-amber-600"></i> Dokumen</span>
                </label>
                <label class="flex items-center gap-2 px-4 py-3 border-2 rounded-xl cursor-pointer transition-all {{ old('type') == 'uniform' ? 'border-purple-500 bg-purple-50' : 'border-slate-200' }}">
                    <input type="radio" name="type" value="uniform" {{ old('type') == 'uniform' ? 'checked' : '' }} class="w-4 h-4">
                    <span><i class="ph ph-t-shirt text-purple-600"></i> Seragam</span>
                </label>
                <label class="flex items-center gap-2 px-4 py-3 border-2 rounded-xl cursor-pointer transition-all {{ old('type') == 'fee' ? 'border-green-500 bg-green-50' : 'border-slate-200' }}">
                    <input type="radio" name="type" value="fee" {{ old('type') == 'fee' ? 'checked' : '' }} class="w-4 h-4">
                    <span><i class="ph ph-money text-green-600"></i> Biaya</span>
                </label>
                <label class="flex items-center gap-2 px-4 py-3 border-2 rounded-xl cursor-pointer transition-all {{ old('type') == 'other' ? 'border-slate-500 bg-slate-50' : 'border-slate-200' }}">
                    <input type="radio" name="type" value="other" {{ old('type') == 'other' ? 'checked' : '' }} class="w-4 h-4">
        ...
        <div>
            ...
            <p class="text-xs text-slate-400 mt-1">Angka lebih kecil ditampilkan lebih dulu.</p>
        </div>
