...
@section('title', 'Detail SPP - ' . $student->name)
...
<div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 mb-6">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div class="flex items-center gap-4">
            <div class="w-16 h-16 bg-gradient-to-br from-sky-400 to-blue-500 rounded-2xl flex items-center justify-center text-white text-2xl font-bold">
                {{ strtoupper(substr($student->name, 0, 1)) }}
            </div>
            <div>
                <h2 class="text-xl font-bold text-slate-800">{{ $student->name }}</h2>
                <p class="text-slate-500">NISN: {{ $student->nisn ?? '-' }} • {{ $student->class_group ?? 'Tidak Ada Kelas' }}</p>
            </div>
        </div>
        @if($unpaidAmount > 0)
        <form action="{{ route('spp.admin.mark_all_paid', $student->nisn ?? $student->id) }}" method="POST">
            @csrf
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
    <div class="bg-white rounded-xl p-5 border border-slate-200 shadow-sm">
        <div class="flex items-center gap-4">
            ...
            <div>
                <p class="text-2xl font-bold text-slate-800">Rp {{ number_format($totalAmount, 0, ',', '.') }}</p>
                <p class="text-sm text-slate-500">Total Semua Tagihan</p>
            </div>
        </div>
    </div>
    <div class="bg-white rounded-xl p-5 border border-slate-200 shadow-sm">
        <div class="flex items-center gap-4">
            ...
            <div>
                <p class="text-2xl font-bold text-red-600">Rp {{ number_format($unpaidAmount, 0, ',', '.') }}</p>
    ...
    <div class="bg-white rounded-xl p-5 border border-slate-200 shadow-sm">
        <div class="flex items-center gap-4">
            ...
            <div>
                <p class="text-2xl font-bold text-green-600">Rp {{ number_format($paidAmount, 0, ',', '.') }}</p>
...
<div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
    <div class="px-6 py-4 border-b border-slate-200 flex justify-between items-center">
        <h3 class="font-bold text-slate-800">Rincian Tagihan per Bulan</h3>
        <a href="{{ route('spp.admin.create') }}?student={{ $student->id }}" 
           class="inline-flex items-center gap-2 px-3 py-1.5 text-sm bg-sky-500 hover:bg-sky-600 text-white rounded-lg transition-colors">
        ...
        <table class="w-full">
            <thead class="bg-slate-50">
                <tr>
                    ...
                    <th class="px-6 py-4 text-center text-xs font-semibold text-slate-500 uppercase">Tanggal Bayar</th>
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50 transition-colors">
                    <td class="px-6 py-4">
                        <span class="font-semibold text-slate-800">{{ $invoice->month }}</span>
                    </td>
                ...
                <tr>
                    <td colspan="5" class="px-6 py-12 text-center text-slate-400">
                        ...
                        <p class="mb-4">Belum ada tagihan untuk siswa ini</p>
                    </td>
                </tr>
                @endforelse
            </tbody>
